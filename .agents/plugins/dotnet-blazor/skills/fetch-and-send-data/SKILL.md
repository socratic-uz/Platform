...
| Mode | Data access |
|------|-------------|
| None (Static SSR) | Server-side: inject domain services / gRPC clients. Use `[StreamRendering]` for loading UX. |
| Server | Server-side: inject domain services. Guard prerender with `??=` + `[PersistentState]`. |
| WebAssembly | Browser-side: `HttpClient` or gRPC-Web client only. No direct server database access. |
| Auto | Both server and browser. Always go through gRPC-Web or service abstraction. |

## Step 2 — Register API & gRPC Clients

Only needed when calling external APIs from Server, or always for WebAssembly/Auto. Server components accessing backend data should inject domain services or gRPC client factories directly.
