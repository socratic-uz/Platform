...
export function resolveSourceRoot(customPath) {
...
export function resolveAnalyzerDll() {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        if (existsSync(candidatePlatform))
...
async function openPage(viewport) {
    ...
    page.on("response", response => {
        if (response.status() >= 400)
            runtime.httpErrors.push({ url: response.url(), status: response.status() });
    });
...
function hostOfflineResponse(path) {
    return {
        content: [{
                type: "text",
                text: JSON.stringify({
...
async function applyFixtureState(page, state, theme) {
    if (state) {
...
async function captureUiSnapshot(page, selector) {
...
async function getSourceContext(componentName, customSourceRoot) {
...
server.registerTool("inspect_ui", {
    ...
    try {
        ...
        const result = await currentPage.evaluate(({ selector: requestedSelector, vp }) => {
            ...
            const isVendorComponent = (node) => {
                const tag = node.tagName.toLowerCase();
                if (tag.startsWith("md-") || tag.startsWith("smart-") || tag.startsWith("quickgrid-") || tag === "svg" || tag === "path")
                    return true;
                const host = node.getRootNode()?.host;
                if (host) {
                    const hostTag = host.tagName.toLowerCase();
                    if (hostTag.startsWith("md-") || hostTag.startsWith("smart-") || hostTag.startsWith("quickgrid-"))
                        return true;
                }
                return false;
            };
            // Forbidden Flexbox layout analysis (Strictly CSS Grid only for application layout code)
            const flexLayoutIssues = [];
            for (const el of allElements) {
                if (isVendorComponent(el))
                    continue;
...
server.registerTool("find_overlaps", {
    ...
    try {
        ...
        const overlaps = await currentPage.evaluate((requestedSelector) => {
            const nodes = Array.from(document.querySelectorAll(requestedSelector))
                .filter((node) => {
            ...
            const result = [];
            const label = (node) => node.id ? `#${node.id}` : `${node.tagName.toLowerCase()}.${node.className.toString().trim().split(/\s+/).join(".")}`;
            for (let i = 0; i < nodes.length; i++) {
                const a = nodes[i].getBoundingClientRect();
                for (let j = i + 1; j < nodes.length; j++) {
                    if (nodes[i].contains(nodes[j]) || nodes[j].contains(nodes[i]))
                        continue;
...
server.registerTool("inspect_component", {
    ...
    try {
        ...
        const browserEvidence = await currentPage.evaluate(({ requestedSelector, requestedComponent, vp }) => {
            const candidates = Array.from(document.querySelectorAll(requestedSelector));
            ...
            if (!element)
                return { error: `Component was not found for selector: ${requestedSelector}` };
            const rect = element.getBoundingClientRect();
            const styles = getComputedStyle(element);
            const issues = [];
        ...
        const componentEvidence = browserEvidence;
        const rawIssues = Array.isArray(componentEvidence.issues) ? componentEvidence.issues : [];
        const issues = rawIssues.map((issue, index) => ({
            ...
            source: typeof sourceContext.component === "object" && sourceContext.component !== null ? sourceContext.component : undefined
        }));
        const report = {
            runId: randomUUID(),
            url: currentPage.url(),
...
server.registerTool("run_ui_scenario", {
    ...
    try {
        ...
        const content = image
            ? [{ type: "text", text }, { type: "image", data: image.toString("base64"), mimeType: "image/png" }]
            : [{ type: "text", text }];
        return { content };
    }
    finally {
...
server.registerTool("analyze_accessibility", {
    ...
    try {
        ...
        const result = await currentPage.evaluate(async ({ target, runTags }) => {
            ...
            if (!context)
                return { error: `Selector not found: ${target}` };
            return axe.run(context, { runOnly: runTags });
        }, { target: selector, runTags: tags });
        const issues = "violations" in result
