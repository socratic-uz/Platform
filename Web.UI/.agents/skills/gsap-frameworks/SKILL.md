...
## Blazor Web App (.NET 10 / WebAssembly / SSR)

Blazor InteractiveAuto / InteractiveWebAssembly uses server-side prerendering followed by client-side hydration.

### Core Principles for Blazor:
1. **Never run GSAP during prerendering**: Only invoke animations in OnAfterRenderAsync(bool firstRender) when irstRender == true.
2. **Scope selectors to component**: Pass the component's root ElementReference to JavaScript and use gsap.context(scope).
3. **Always clean up on component destruction**: Implement IAsyncDisposable and call ctx.revert() via JS module disposal.
4. **Prevent SSR FOUC**: Use utoAlpha or initial CSS so elements don't flicker or flash during hydration handoff.

### Collocated Module Pattern (Component.razor + Component.razor.js)
...
<div @ref="_container" class="kiosk-container">
    <div class="kiosk-card">Item 1</div>
    <div class="kiosk-card">Item 2</div>
</div>
...
@code {
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            _module = await JS.InvokeAsync<IJSObjectReference>("import", "./Pages/KioskView.razor.js");
            await _module.InvokeVoidAsync("initKioskMotion", _container);
        }
    }
    ...
    public async ValueTask DisposeAsync()
    {
        if (_module is not null)
        {
            try
            {
                await _module.InvokeVoidAsync("cleanupMotion");
            }
...
export function initKioskMotion(container) {
    if (!container) return;
    
    // All selectors inside the callback are strictly scoped to container
    ctx = gsap.context(() => {
        gsap.from(".kiosk-card", {
            autoAlpha: 0,
            y: 30,
            scale: 0.94,
            duration: 0.35,
...
export function cleanupMotion() {
    ctx?.revert(); // Kills tweens, removes inline styles, prevents memory leaks
}
`
