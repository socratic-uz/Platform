...
{
    public static async Task IngestDataAsync(IServiceProvider services, params IIngestionSource[] sources)
    {
    ...
    public async Task IngestDataAsync(IIngestionSource source)
    {
        ...
        var documentsForSource = new List<IngestedDocument>();
        await foreach (var doc in documentsCollection.GetAsync(doc => doc.SourceId == sourceId, top: int.MaxValue))
            documentsForSource.Add(doc);
        ...
        async Task DeleteChunksForDocumentAsync(IngestedDocument document)
        {
            ...
            var chunksToDelete = new List<IngestedChunk>();
            await foreach (var chunk in chunksCollection.GetAsync(record => record.DocumentId == documentId, int.MaxValue))
