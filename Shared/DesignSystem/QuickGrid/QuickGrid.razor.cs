...
public partial class QuickGrid<TGridItem> : IAsyncDisposable
{
    ...
    /// <summary>
    /// Optionally links this <see cref="SmartGrid{TGridItem}"/> instance with a <see cref="PaginationState"/> model,
    ...
    protected ElementReference _tableReference;
    protected Virtualize<(int, TGridItem)>? _virtualizeComponent;
    protected int _ariaBodyRowCount;
    protected ICollection<TGridItem> _currentNonVirtualizedViewItems = Array.Empty<TGridItem>();
    ...
    protected IAsyncQueryExecutor? _asyncQueryExecutor;
    ...
    protected readonly InternalGridContext<TGridItem> _publicGridContext;
    protected readonly List<ColumnBase<TGridItem>> _columns;
    protected bool _collectingColumns; // Columns might re-render themselves arbitrarily. We only want to capture them at a defined time.

    // Tracking state for options and sorting
    protected ColumnBase<TGridItem>? _displayOptionsForColumn;
    protected ColumnBase<TGridItem>? _sortByColumn;
    protected bool _sortByAscending;
    protected bool _checkColumnOptionsPosition;

    // The associated ES6 module, which uses document-level event listeners
    protected IJSObjectReference? _jsModule;
    protected IJSObjectReference? _jsEventDisposable;

    // Caches of method->delegate conversions
    protected readonly RenderFragment _renderColumnHeaders;
    protected readonly RenderFragment _renderNonVirtualizedRows;
    ...
    protected int? _lastRefreshedPaginationStateHash;
    protected object? _lastAssignedItemsOrProvider;
    protected CancellationTokenSource? _pendingDataLoadCancellationTokenSource;

    // If the PaginationState mutates, it raises this event. We use it to trigger a re-render.
    protected readonly EventCallbackSubscriber<PaginationState> _currentPageItemsChanged;

    // If the SmartGrid is disposed while the JS module is being loaded, we need to avoid calling JS methods
    protected bool _wasDisposed;

    protected bool _firstRefreshDataAsync = true;

    /// <summary>
    /// Exposes the internal grid context for derived components to cascade.
    /// </summary>
    protected InternalGridContext<TGridItem> GridContext => _publicGridContext;

    /// <summary>
    /// Exposes the collected columns to derived components.
    /// </summary>
    protected IReadOnlyList<ColumnBase<TGridItem>> Columns => _columns;

    /// <summary>
    /// Exposes the currently loaded items (non-virtualized) to derived components.
    /// </summary>
    protected IEnumerable<TGridItem> CurrentItems => _currentNonVirtualizedViewItems;

    /// <summary>
    /// Constructs an instance of <see cref="SmartGrid{TGridItem}"/>.
    /// </summary>
    public QuickGrid()
    {
        _columns = new();
        _publicGridContext = new(this);
        ...
        columnsFirstCollectedSubscriber.SubscribeOrMove(_publicGridContext.ColumnsFirstCollected);
    }
    ...
    protected override Task OnParametersSetAsync()
    {
        ...
        {
            throw new InvalidOperationException($"{nameof(QuickGrid<TGridItem>)} requires one of {nameof(Items)} or {nameof(ItemsProvider)}, but both were specified.");
        }
        ...
        if (dataSourceHasChanged)
        {
            ...
            {
                // IServiceProvider ��� disposed - ��� ��������� � ��������� ��������� Blazor WebAssembly
                // ���������� ��� async executor (����� �������������� ���������� LINQ ������)
                _asyncQueryExecutor = null;
            }
        }
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            _jsModule = await JS.InvokeAsync<IJSObjectReference>("import", "./_content/QuickGrid/QuickGrid.razor.js");
            if (_wasDisposed)
            {
    ...
    public void AddColumn(ColumnBase<TGridItem> column, SortDirection? initialSortDirection, bool isDefaultSortColumn)
    {
    ...
    protected void StartCollectingColumns()
    {
        _columns.Clear();
        _collectingColumns = true;
    }

    protected void FinishCollectingColumns()
    {
    ...
    protected async Task RefreshDataCoreAsync()
    {
        ...
        if (_virtualizeComponent is not null)
        {
            ...
            // (1) It won't know to update its own public state if the provider output has changed
    ...
    protected async ValueTask<ItemsProviderResult<(int, TGridItem)>> ProvideVirtualizedItems(ItemsProviderRequest request)
    {
        _lastRefreshedPaginationStateHash = Pagination?.GetHashCode();
    ...
    protected async ValueTask<GridItemsProviderResult<TGridItem>> ResolveItemsRequestAsync(GridItemsProviderRequest<TGridItem> request)
    {
    ...
    protected string AriaSortValue(ColumnBase<TGridItem> column)
        => _sortByColumn == column
            ? (_sortByAscending ? "ascending" : "descending")
            : "none";

    protected string? ColumnHeaderClass(ColumnBase<TGridItem> column)
        => _sortByColumn == column
    ...
    protected static string? ColumnClass(ColumnBase<TGridItem> column) => column.Align switch
    {
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        {
            // Disposal can race with circuit shutdown and JS runtime cancellation.
            // Ignore cancellation to avoid bringing down the Blazor circuit.
        }
        catch (JSException)
        {
            // JS module may already be partially torn down during route changes.
            // Ignore to keep renderer disposal resilient.
        }
    }
}
