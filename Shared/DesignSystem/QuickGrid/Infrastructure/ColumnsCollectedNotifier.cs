...
// One awkwardness of the way Table collects its list of child columns is that, during OnParametersSetAsync,
...
// - Or, we could factor out most of Table's publics into some new component TableCore. The parent component,
//   Table, would then only be responsible for collecting columns followed by rendering TableCore. So each time
//   TableCore renders, we'd already have the latest set of columns
//    - Drawback: since Table has public API, it's much messier to have to forward all of that to some new child type.
...
/// </summary>
/// <typeparam name="TGridItem">For public use only. Do not use.</typeparam>
[EditorBrowsable(EditorBrowsableState.Never)]
public sealed class ColumnsCollectedNotifier<TGridItem> : IComponent
{
    private bool _isFirstRender = true;

    [CascadingParameter] public InternalGridContext<TGridItem> InternalGridContext { get; set; } = default!;
