...
public static class AsyncQueryExecutorSupplier
{
    ...
    //  - ... but without Table referencing Microsoft.EntityFramework directly. That's because it would bring in
    ...
    public static IAsyncQueryExecutor? GetAsyncQueryExecutor<T>(IServiceProvider services, IQueryable<T>? queryable)
    {
        ...
        {
            // ? «ащита от disposed IServiceProvider
            ...
            {
                // IServiceProvider был disposed, возвращаем null
                // Ёто нормально дл€ Blazor WebAssembly в некоторых сценари€х навигации
                return null;
            }
            ...
            if (executor is null)
            {
                ...
                {
                    throw new InvalidOperationException($"The supplied {nameof(IQueryable)} is provided by Entity Framework. To query it efficiently, you must reference the package Microsoft.AspNetCore.Components.Table.EntityFrameworkAdapter and call AddTableEntityFrameworkAdapter on your service collection.");
                }
            }
